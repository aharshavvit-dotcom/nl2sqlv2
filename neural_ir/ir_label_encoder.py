...
METRIC_EXPRESSION_TYPES = ["column", "count_star", "product_revenue_expression", "none"]
...
class IRLabelEncoder:
    ...
    def encode(self, query_ir: dict[str, Any], schema_items: dict[str, Any]) -> dict[str, int]:
        ...
        aggregation = str((metric or {}).get("aggregation") or "NONE").upper()
        ...
        return {
            ...
            "metric_column_index": _column_index(schema_items, (metric or {}).get("table"), (metric or {}).get("column")),
            "metric_expression_type_label": self._label("metric_expression_type", _metric_expression_type(metric), "none"),
            "dimension_column_index": _column_index(schema_items, (dimension or {}).get("table"), (dimension or {}).get("column")),
            "date_column_index": _column_index(schema_items, (date_filter or {}).get("date_table"), (date_filter or {}).get("date_column")),
            ...
            "filter_column_index": _column_index(schema_items, (ir_filter or {}).get("table"), (ir_filter or {}).get("column")),
    ...
    def decode(self, predictions: dict[str, int], schema_items: dict[str, Any]) -> dict[str, Any]:
        ...
        metric_expression_type = self._decode_label("metric_expression_type", predictions.get("metric_expression_type_label", 3))
...
def _column_index(schema_items: dict[str, Any], table: Any, column: Any) -> int:
    if not table or not column or column == "*":
        return -1
    target = (str(table), str(column))
    for idx, item in enumerate(schema_items.get("columns", [])):
        if (item.get("table"), item.get("column")) == target:
            return idx
    return -1
