...
from .trainer import HEAD_TO_LABEL
...
class OptionAIREvaluator:
    ...
    def evaluate(self, model, data_loader, label_encoder) -> dict[str, Any]:
        model.eval()
        ...
        with torch.no_grad():
            for batch in data_loader:
                outputs = model(batch["question_ids"], batch["schema_ids"], batch["question_mask"], batch["schema_mask"])
                pred_indices = {head: logits.argmax(dim=-1).cpu().tolist() for head, logits in outputs.items()}
